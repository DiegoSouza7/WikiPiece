...
CREATE TABLE frutas (
  ...
  PRIMARY KEY (id),
  CONSTRAINT fk_fruta_imagens FOREIGN KEY (imagem_id) REFERENCES imagens (id) ON DELETE CASCADE
);
...
CREATE TABLE tripulacao (
  ...
  "imagem_id" INTEGER,
  PRIMARY KEY (id),
  CONSTRAINT fk_tripulacao_imagem FOREIGN KEY (imagem_id) REFERENCES imagens (id) ON DELETE CASCADE
);
